...
// 抽盤面------------------
//
//	@param slotType	- 遊戲類型 : 0 - NG ; 1 - FG；3 - NG必中FG
//	@return pt 	- 盤面
//	@return pos	- debug用，reelpos
	...
	for id, payLine := range PayLines {
		...
		if len(symbolPayout[firstSymbol]) > count {
			if symbolPayout[firstSymbol][count] > 0 {
				...
				winfloat, _ := winpoint.Div(decimal.NewFromInt(int64(MAX_PAYLINE))).Round(2).Float64()
...
// Spin
//
//	@Param	retrigger	- 機率使用，server端請輸入0
//	@return round 回合資料
...
func accWildPrize(isgetww [5][5]int, pt Reels) ([]float64, decimal.Decimal) {
