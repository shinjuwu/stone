...
//	@rtp	- 回合現行rtp
//	@Result	- 每一轉資訊：包含主遊戲及免費遊戲
//	@FreeSpin	- 免費遊戲次數
//	@TotalBet	- 總下注
//	@TotalPoint	- 總贏分
//	@TotalPoint_decimal	- 總贏分的decimal格式
type Rounds struct {
	Rtp             string          `json:"rtp"`
...
//	@payReel	- 回合盤面
//	@Windetail	- 贏分明細
//	@WildPrize	- Wild中獎獎項倍率 e.g.[1,7,10]表示得到 1倍、7倍、10倍獎項
//	@ReelPosition	- 該局中獎的Reel Index -便於debug mode
//	@Point		- 該局得分
//	@Bet		- 下注
//	@TotalPoint	- 總贏分
//	@Point_Deci	- 該局得分的decimal格式（計算用）
//	@TotalPoint_Deci	- 總贏分的decimal格式（計算用）
type Records struct {
	Id              int             `json:"id"`
