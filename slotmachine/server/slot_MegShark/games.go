...
func NewGames(ng, fg *WayGames) *Games {
	return &Games{
		ng_game: ng,
		fg_game: fg,
	}
...
type Records struct {
	...
	Pickem          []string        `json:"pickem"`        //暫時沒有用
	PreReel         Reels           `json:"originalReels"` //if slot type is 0 , then ww reel is nil.
	WwReel          Reels           `json:"wwReels"`
	WwSym           int             `json:"wwSym"`
	Windetail       []Windetail     `json:"windetail"`
	ReelPosition    []int           `json:"reelposition"` //for debug
	Point           float64         `json:"point"`        //顯示用
	Bet             float64         `json:"bet"`
	TotalPoint      float64         `json:"TotalPoint"`         //顯示用
	Point_Deci      decimal.Decimal `json:"point_decimal"`      //機率計算用
	TotalPoint_Deci decimal.Decimal `json:"totalpoint_decimal"` //機率計算用
}
type State uint16
