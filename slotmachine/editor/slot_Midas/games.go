...
type Rounds struct {
	...
	TotalPoint      float64         `json:"totalpoint"`         //顯示用
	TotalPoint_Deci decimal.Decimal `json:"totalpoint_decimal"` //計算用
}
...
//	@WWMulti 	- 回合加成倍數
//	@windetail	- 贏分明細
//	@ScatterInfo	- 該回合Scatter資訊：包含Scatter個數及抽取FG次數
//	@FgWildReel	- 免費遊戲中的固定wild資訊：包含前一轉固定狀態及當轉狀態
//	@ReelPosition	- 該局中獎的Reel Index -便於debug mode
//	@Point		- 該局得分
//	@Bet		- 下注
//	@TotalPoint	- 總贏分
//	@Point		- 該局得分的decimal格式
//	@TotalPoint	- 總贏分的decimal格式
type Records struct {
	Id              int             `json:"id"`
	...
	PayReel         Reels           `json:"PayReels"` //if slot type is 0 , then ww reel is nil.
	WWMulti         int             `json:"wwMulti"`
	Windetail       []Windetail     `json:"windetail"`
	ScatterInfo     SfInfo          `json:"scatterInfo"`
	FgWildReel      WildInfo        `json:"WildReel"`
	ReelPosition    []int           `json:"reelposition"` //for debug
	Point           float64         `json:"point"`        //顯示用
	Bet             float64         `json:"bet"`
	TotalPoint      float64         `json:"TotalPoint"`         //顯示用
	Point_Deci      decimal.Decimal `json:"point_decimal"`      //計算用
	TotalPoint_Deci decimal.Decimal `json:"totalpoint_decimal"` //計算用
}
type State uint16
