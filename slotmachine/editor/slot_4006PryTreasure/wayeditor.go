...
import (
	...
	"slotEditor/constant"
	"slotEditor/editor"
	server "slotserver/slot_4006PryTreasure"
	"strconv"
...
func (t *WayEditor) wayServerRunOnce() {
	...
	win, _ = curRound.TotalPoint_Deci.Round(2).Float64()
	win_ng, _ := decimal.NewFromFloat(curRound.Result[0].TotalPoint).Round(2).Float64()
	var win_bg float64
	t.CurPos = curRound.Result[0].ReelPosition
	for i := range curRound.Result {
		if i == 0 {
			t.StatNormalWin += win_ng
			t.CurNG = win_ng
		}
		if i > 0 {
			win_bg, _ = decimal.NewFromFloat(curRound.Result[len(curRound.Result)-1].TotalPoint).Round(2).Float64()
			t.CurBG = win_bg
		}
	}
	t.StatBonusWin += win_bg
	...
	t.FgGetWin += len(curRound.Result) - 1
	//
...
func (t *WayEditor) wayRunOnce() {
	...
	win, _ = curRound.TotalPoint_Deci.Round(2).Float64()
	win_ng, _ := curRound.Result[0].TotalPoint_Deci.Round(2).Float64()
	t.CurPos = curRound.Result[0].ReelPosition
	...
	for i := range curRound.Result {
		...
		if i > 0 {
			win_bg, _ = decimal.NewFromFloat(curRound.Result[len(curRound.Result)-1].TotalPoint).Float64()
			t.CurBG = win_bg
		}
	...
	for i := 0; i < len(round.Result); i++ {
		if len(round.Result) > 1 {
			if i == 0 {
				...
				str += fmt.Sprintf("\nBG第%d層:%d/%d\n", i, round.Result[i].Stages, len(round.Result)-1)
			}
		} else {
			str += "NG\n"
		}
		preReel := round.Result[i].PayReel
		if len(preReel) == 0 {
			return str
		}
		if i == 0 {
			for j := 0; j < rowNum+2; j++ {
				for i := 0; i < colNum; i++ {
					symbol := int(preReel[i][j])
					tmpstr := symbolNameList[symbol] + " "
					str = str + tmpstr
				}
				str = str + "\n"
			}
			str += fmt.Sprintf("\nwild expand:%+v", round.Result[i].NgWildExpand)

		} else {
			for j := range preReel[0] {
				symbol := preReel[0][j]
				tmpstr := fmt.Sprintf("%.1f", symbol)
				str = str + tmpstr + "	"
			}
			str = str + "\n"

			if len(round.Result[i].BgExtra) > 0 {
				if round.Result[i].BgExtra[0] == 0 {
					str += "Game over\n"
				} else {
					str += "Up\n"
				}
		...
		if i == 0 && round.Result[0].Point > 0 {
			...
			for _, v := range round.Result[0].Windetail {
				str += fmt.Sprintf("\nSymbol: %+v 得到 %+v Way %+v 連線 得分:%+v", v.Symbol, v.WinWay, v.Match, v.Multi)
			}
			str += "\n"
		}
		if i == 0 {
		...
		if round.Result[i].Case.IsBonus() {
			str += "中獎內容:"
